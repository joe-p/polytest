...
The `version` field is **required** and specifies the version of polytest that the configuration file is compatible with. The version **must** be in MAJOR.MINOR.PATCH format (e.g., `"0.4.0"`). Polytest will validate that:
1. The MAJOR.MINOR components match the binary version exactly
2. The binary's PATCH version is greater than or equal to the config's PATCH version

This ensures compatibility between the configuration file and the polytest tool while allowing newer polytest binaries to work with configurations written for older patch versions.

If the version field is missing, polytest will exit with an error message asking you to add it. If the version is not in MAJOR.MINOR.PATCH format, polytest will exit with an error. If the MAJOR.MINOR version does not match the binary version, or if the config's PATCH version is higher than the binary's PATCH version, polytest will exit with an error message indicating the version mismatch.
...
```json
{
  "name": "Shapes Test Plan",
  "package_name": "shapes",
  "version": "0.4.0"  // Must be MAJOR.MINOR.PATCH format
}
```
